...
import StudentModal from '@/lib/components/modals/student.modal'
import { useAppDispatch, useAppSelector } from '@/lib/store/hooks'
import { deleteInstituteStudentById, fetchInstituteStudent } from '@/lib/store/institute/student/institute-student-slice'
...
const InstituteStudent = () => {
  ...
  const { students } = useAppSelector((state) => state.instituteStudent)
  ...
  const deleteStudent = (id:string) =>{
    id && dispatch(deleteInstituteStudentById(id))
  }

  const filteredData = students.filter((student)=>student.studentName?.toLowerCase().includes(searchedText.toLowerCase()))
              ...
              <table className="min-w-full divide-y divide-gray-300">
                <thead>
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Address</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Enrolled Date</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-300">
                  {filteredData.map((s) => (
                      <tr key={s.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                        <td className="p-5 text-sm font-medium text-gray-900">{s.id}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{s?.studentName}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{s?.studentEmail}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{s?.studentPhoneNumber}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{s?.studentAddress}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{s?.enrolledDate}</td>
                        <td className="p-5">
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={()=>deleteStudent(s.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
