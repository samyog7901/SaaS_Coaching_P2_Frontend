...
import TeacherModal from '@/lib/components/modals/teacher.modal'
...
const InstituteTeacher = () => {
  ...
  const { teachers } = useAppSelector((state) => state.instituteTeacher)
  ...
  // const getCourseName = (teacherId: string | null | undefined) => {
  //   if (!teacherId || courses.length === 0) return "No course assigned"; // Handle null
  //   const course = courses.find(c => c.teacherId === teacherId.toString());
  //   return course ? course.courseName : "No course assigned";
  // };
  

  const filteredData = teachers.filter((teacher)=>teacher.teacherName?.toLowerCase().includes(searchedText.toLowerCase()))
          ...
          {isModalOpen && <TeacherModal closeModal={closeModal} />}
            <div className="min-w-full inline-block align-middle">
              ...
              <div className="flex justify-between items-center gap-4 mb-6">
                ...
                {/* Add Category Button */}
                <button onClick={openModal} className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white text-sm font-medium rounded-full shadow-md hover:bg-green-500 focus:ring-2 focus:ring-amber-400 transition-all duration-200">
              ...
              <table className="min-w-full divide-y divide-gray-300">
                <thead>
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Email</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Phone Number</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">JoinedAt</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Course</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-300">
                  {filteredData.map((t) => (
                      <tr key={t.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                        <td className="p-5 text-sm font-medium text-gray-900">{t.id}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{t?.teacherName}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{t?.teacherEmail}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{t?.teacherPhoneNumber}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{t?.teacherJoinedDate}</td>
                        <td className="p-5 text-sm font-medium text-gray-900">{t?.courseName}</td>
                        <td className="p-5">
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={()=>deleteTeacher(t.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
