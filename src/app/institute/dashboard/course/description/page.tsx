...
// import { useAppDispatch } from "@/lib/store/hooks"
// import { updateCategory } from "@/lib/store/institute/course/course.slice"

const CourseDescriptionPopup = ({ course }: { course: ICoursePostData }) => {
  const [showPopup, setShowPopup] = useState(false)
  const [isEditing, setIsEditing] = useState(false)
  const [newDescription, setNewDescription] = useState(course.courseDescription || "")

  const dispatch = useAppDispatch()

  const handleSave = () => {
    // 👉 Call Redux or API update here
    // dispatch(updateCategory({ id: course.id, categoryDescription: newDescription }))
    ...
    <div className="relative inline-block">
      ...
      {showPopup && (
        <div className="absolute left-0 mt-2 w-72 p-4 bg-white border rounded-lg shadow-lg z-50">
          <h4 className="font-semibold text-amber-600">{course.courseName}</h4>
