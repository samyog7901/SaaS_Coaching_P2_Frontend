"use client"
import CourseModal from '@/lib/components/modals/course.modal'
import { useAppDispatch, useAppSelector } from '@/lib/store/hooks'
import { fetchCategories } from '@/lib/store/institute/category/categorySlice'
import { deleteInstituteCourseById, fetchInstituteCourse } from '@/lib/store/institute/course/institute-course-slice'
import React, { useEffect, useState } from 'react'
import CourseDescriptionPopup from './description/page'
...
const InstituteCourses = () => {
  ...
  const deleteCourse = (id: string) => {
    id && dispatch(deleteInstituteCourseById(id))
  }
  ...
  // ✅ filter courses instead of categories
  const filteredData = courses.filter((course) =>
    course.courseName?.toLowerCase().includes(searchedText.toLowerCase()) ||
    course.courseDescription?.toLowerCase().includes(searchedText.toLowerCase())
  )
            ...
            <div className="flex justify-between items-center gap-4 mb-6">
              ...
              <div className="relative w-80">
                ...
                <input
                  ...
                  className="block w-full h-11 pl-12 pr-4 rounded-full border border-gray-300 bg-white shadow-sm text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 transition-all duration-200"
                />
              </div>
              ...
              <button
                onClick={openModal}
                className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white text-sm font-medium rounded-full shadow-md hover:bg-green-500 focus:ring-2 focus:ring-amber-400 transition-all duration-200"
              >
          ...
          {filteredData.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-300">
                <thead>
                  <tr>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">ID</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Name</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Price</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Description</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Duration</th>
                    <th className="px-5 py-3 text-left text-sm font-semibold text-gray-900">Course Level</th>
                ...
                <tbody className="divide-y divide-gray-300">
                  {filteredData.map((course) => (
                    <tr key={course.id} className="bg-white hover:bg-gray-50 transition-all duration-500">
                      <td className="p-5 text-sm font-medium text-gray-900">{course.id}</td>
                      <td className="p-5 text-sm font-medium text-gray-900">{course?.courseName}</td>
                      <td className="p-5 text-sm font-medium text-gray-900">{course?.coursePrice}</td>
                      <td><CourseDescriptionPopup course = {course}/></td>
                      <td className="p-5 text-sm font-medium text-gray-900">{course?.courseDuration}</td>
                      <td className="p-5 text-sm font-medium text-gray-900">{course?.courseLevel}</td>
                      <td className="p-5 text-sm font-medium text-gray-900">{getCategoryName(course.categoryId)}</td>
                      <td className="p-5">
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={()=>deleteCourse(course.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
